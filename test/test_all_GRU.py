...
for batch_size in batch_sizes:
    ...
    checkpoint_path = 'checkpoints/best_model_0110_four_layers_GRU_{}.h5'.format(batch_size)
    ...
    model.szve('0110_four_layers_GRU.h5')
    # 儲存損失
    ...
    if not os.path.exists('plots'):
        os.makedirs('plots')
    img_path = "plots/0110_four_layers_GRU_{}.png".format(batch_size)
...
plt.title('Training Loss Comparison for Different Batch Sizes (GRU)')
plt.xlabel('Epochs')
plt.ylabel('Loss(m)')
plt.legend()
plt.grid(True)
...
if not os.path.exists('plots'):
    os.makedirs('plots')
plt.savefig('plots/0110_four_layers_GRU.png')
plt.show()
