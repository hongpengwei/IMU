...
# 指定資料夾路徑
folder_path = 'C:\\Users\\HONG\\Desktop\\IMU\\test\\distance'
...
for file in files:
    ...
    print(step)
    output_file_path_copy = "C:\\Users\\HONG\\Desktop\\IMU\\test\\distance\\{}.csv".format(j)
...
for batch_size in batch_sizes:
    ...
    checkpoint_path = 'checkpoints/best_model_0110_four_layers_biGRU_{}.h5'.format(batch_size)
    ...
    # 創建一個新的模型
    model = Sequential()
    model.add(Bidirectional(GRU(256, return_sequences=True), input_shape=(sequence_length, num_features)))
    ...
    model.save('0110_four_layers_biGRU.h5')
    # 儲存損失
    ...
    if not os.path.exists('plots'):
        os.makedirs('plots')
    img_path = "plots/0110_four_layers_biGRU_{}.png".format(batch_size)
...
plt.title('Training Loss Comparison for Different Batch Sizes (biGRU)')
plt.xlabel('Epochs')
plt.ylabel('Loss(m)')
plt.legend()
plt.grid(True)
...
if not os.path.exists('plots'):
    os.makedirs('plots')
plt.savefig('plots/0110_four_layers_biGRU.png')
plt.show()
