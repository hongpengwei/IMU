...
for batch_size in batch_sizes:
    ...
    checkpoint_path = 'checkpoints/best_model_0110_four_layers_lstm_{}.h5'.format(batch_size)
    ...
    model.add(LSTM(256, return_sequences=True))  
    model.add(Dropout(0.5))
    model.add(LSTM(256))  # 最後一層LSTM，不需要return_sequences=True，默認為False
    model.add(Dropout(0.5))
    model.add(Dense(1))
    ...
    model.szve('0110_four_layers_lstm.h5')
    # 儲存損失
    ...
    if not os.path.exists('plots'):
        os.makedirs('plots')
    img_path = "plots/0110_four_layers_lstm_{}.png".format(batch_size)
...
plt.title('Training Loss Comparison for Different Batch Sizes (LSTM)')
plt.xlabel('Epochs')
plt.ylabel('Loss(m)')
plt.legend()
plt.grid(True)
...
if not os.path.exists('plots'):
    os.makedirs('plots')
plt.savefig('plots/0110_four_layers_lstm.png')
plt.show()
