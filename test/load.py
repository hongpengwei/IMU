...
for file_name in os.listdir(folder_path):
    if file_name.endswith('.csv'):
        ...
        # 分割資料成 200 筆一組的序列
        for i in range(0, len(data) - sequence_length, sequence_length):
            seq = data[['gyro_x', 'gyro_y', 'gyro_z', ' worldXAccel', 'worldYAccel', 'worldZAccel']].values[i:i+sequence_length]
...
checkpoint_path_32 = 'checkpoints/best_1222_new_32_only_linacc.h5'
...
model_32 = load_model('checkpoints\\best_model_1221_only_lin_acc_three_layers_32.h5')
...
model_32.save('best_model_1222_new.h5')
...
plt.legend()
plt.title('Training and Validation Loss for Batch Sizes 32')
plt.grid(True)

# Save the plot
if not os.path.exists('plots'):
    os.makedirs('plots')
plt.savefig('plots/1222_32_only_linacc.png')
